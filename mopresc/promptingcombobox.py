...
class PromptingComboBox(wx.ComboBox) :
    ...
    def EvtText(self, event):
        ...
        for choice in self.choices :
            if currentText in choice:#if choice.startswith(currentText):
                self.ignoreEvtText = True
                self.SetValue(choice)
                start = choice.find(currentText) + 1
                self.SetInsertionPoint(start)#self.SetInsertionPoint(len(currentText))
                self.SetMark(start, len(choice))
                found = True
                break
        if not found:
            event.Skip()
...
class TrialPanel(wx.Panel):
    def __init__(self, parent):
        ...
        cb = PromptingComboBox(self, "default value", choices) 
