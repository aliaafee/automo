...
class PatientSearchPanel(wx.Panel):
    ...
    def _search(self, str_search):
        items = self.session.query(Patient)

        if str_search != "":
            items = items.filter(
                or_(
                    Patient.hospital_no.like("%{0}%".format(str_search)),
                    Patient.name.like("%{0}%".format(str_search))
                )
            )
    ...
    def _on_patient_selected(self, event):
        ...
        event = PatientChangedEvent(object=selected_patient)
    ...
    def _patient_decorator(self, patient, query_string):
        ...
        return html.format(
            str_hospital_no,
            str_name,
            self._non_breaking(patient.age()),
            patient.sex
        )
